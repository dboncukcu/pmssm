...
for par in ["abs(chi1pm)-abs(chi10)","g-abs(chi10)","t1-abs(chi10)","b1-abs(chi10)","lcsp-abs(chi10)","abs(chi20)-abs(chi10)"]:
    ...
    pmssm_flipbook.survivalProbability2D(f"{par}:abs(chi10)", analysis="cms_sus_18_004,cms_sus_20_001,cms_sus_21_007,cms_sus_21_007_mb",customName="_3",legendAddition="+ SUS-21-007")
    pmssm_flipbook.survivalProbability2D(f"{par}:abs(chi10)", analysis="cms_sus_18_004,cms_sus_20_001,cms_sus_21_007,cms_sus_21_007_mb,cms_sus_21_006",customName="_4",legendAddition="+ SUS-21-006")
    pmssm_flipbook.survivalProbability2D(f"{par}:abs(chi10)", analysis="combined",customName="_5",legendAddition="+ SUS-19-006")
    pmssm_flipbook.survivalProbability2D(
        f"{par}:abs(chi10)", 
        analysis="combined",
        moreconstraints=["Omegah2<=0.132"],  customName="_6",legendAddition="+ DM relic density")
    pmssm_flipbook.survivalProbability2D(
        f"{par}:abs(chi10)", 
        analysis="combined",
        moreconstraints=["Omegah2<=0.132","abs(dd_exclusion_pval_withlz)>=0.05"],customName="_7", legendAddition="+DM direct detection")
       
    pmssm_flipbook.survivalProbability2D(
        f"{par}:abs(chi10)", 
        analysis="combined",
        moreconstraints=["Omegah2<=0.132","abs(dd_exclusion_pval_withlz)>=0.05","deltaEW<=500"],customName="_8",
        legendAddition="+ #Delta_{EW}<200")
    del pmssm_flipbook
